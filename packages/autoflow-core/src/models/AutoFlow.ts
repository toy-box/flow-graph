...
// import { clone, isNum, isObj, isArr, uid } from '@toy-box/toybox-shared'
// import { Flow, FlowNodeType, FlowNode } from '@toy-box/flow-graph'
...
// const BASE_SIZE = 30
// const STANDARD_SIZE = BASE_SIZE * 2
...
//     //     const data = meta.children.find(
//     //       (child) => child.key === nodeId || child.key === flowData.id
//     //     );
...
//   ) => {
//     const fork =
//       metaType === FlowMetaType.DECISION || metaType === FlowMetaType.WAIT
//         ? flowData
//         : undefined
...
//         this.initMetaFields(meta.flowType, meta, MetaFieldType.EDIT, nodeId)
//       } else if (meta.nextValueConnector?.targetReference === nodeId) {
//         meta.nextValueConnector.targetReference =
//           meta.id !== targetId ? targetId : null
//         this.initMetaFields(meta.flowType, meta, MetaFieldType.EDIT, nodeId)
//       } else if (
//         meta.flowType === FlowMetaType.DECISION ||
//         meta.flowType === FlowMetaType.WAIT
//       ) {
...
//         this.metaFlowDatas = this.metaFlowDatas.filter(
//           (meta) => meta.id !== nodeId
//         )
//         const currnetMeta = this.metaFlowDatas.find(
//           (meta) => meta.id === targetReferenceId
//         )
//         const rules = flowData?.rules || flowData.waitEvents
//         rules?.forEach((rule) => {
//           const currnetRuleMeta = this.metaFlowDatas.find(
//             (meta) => meta.id === rule.connector?.targetReference
//           )
//           if (
//             currnetRuleMeta &&
//             fork?.connector?.targetReference !== rule.connector?.targetReference
//           )
//             this.removeFlowDataFunc(
//               currnetRuleMeta?.id,
//               currnetRuleMeta.flowType,
//               currnetRuleMeta,
//               true,
//               fork
//             )
...
//         if (
//           currnetMeta &&
//           fork?.connector?.targetReference !== targetReferenceId
//         )
//           this.removeFlowDataFunc(
//             currnetMeta?.id,
//             currnetMeta.flowType,
//             currnetMeta,
//             true,
//             fork
//           )
...
//           (meta) => meta.id !== nodeId
//         )
//         const nextMeta = this.metaFlowDatas.find(
//           (meta) => meta.id === nextTargetId
//         )
//         if (
//           flowData.defaultConnector?.targetReference !==
//             fork?.connector?.targetReference &&
//           flag
//         ) {
//           const currentDefault = this.metaFlowDatas.find(
//             (meta) => meta.id === flowData.defaultConnector?.targetReference
//           )
//           if (currentDefault)
//             this.removeFlowDataFunc(
//               currentDefault?.id,
//               currentDefault.flowType,
//               currentDefault,
//               true,
//               fork
//             )
...
//           const defaultMeta = this.metaFlowDatas.find(
//             (meta) => meta.id === defaultTargetId
//           )
//           if (defaultMeta)
//             this.removeFlowDataFunc(
//               defaultMeta?.id,
//               defaultMeta.flowType,
//               defaultMeta,
//               true,
//               fork
//             )
...
//           flag
//         ) {
//           const baseMeta = this.metaFlowDatas.find(
//             (meta) => meta.id === flowData.connector?.targetReference
//           )
//           if (baseMeta)
//             this.removeFlowDataFunc(
//               baseMeta?.id,
...
//     const data = flowData
//     const flowNode: FlowNode | undefined = this.flowNodes.find(
//       (node) => node.id === nodeId
//     )
...
//           if (
//             ft.defaultConnector?.targetReference ===
//             flowMeta?.defaultConnector?.targetReference
//           ) {
//             fork = ft
//             return ft
//           }
//         } else if (
//           ft.connector?.targetReference ===
//           flowMeta?.defaultConnector?.targetReference
//         ) {
//           fork = ft
//           return ft
...
//           if (
//             ft.defaultConnector?.targetReference ===
//             flowMeta?.connector?.targetReference
//           ) {
//             fork = ft
//             return ft
//           }
//         } else if (
//           ft.connector?.targetReference === flowMeta?.connector?.targetReference
//         ) {
//           fork = ft
//           return ft
//         }
...
//     // this.flow = new Flow()
//     const loopBase = this.flowNodes.find(
//       (node) => node.loopEndTarget === flowNode?.id
//     )
//     const decisionBase = this.flowNodes.find(
//       (node) => node.loopEndTarget === flowNode?.id
//     )
//     const nodes = clone(this.flowNodes)
//     // const obj = {
...
//     metaType: FlowMetaType,
//     flowData: FlowMetaParam,
//     currentUpdataData?: FlowMetaParam
//   ) => {
...
//       const forkBeginOfEndNode = this.flowNodes.find(
//         (node) => node.decisionEndTarget === flowNode?.id
//       )
//       const base = this.flowNodes.find((node) => {
//         return (
//           node?.targets?.length === 1 &&
//           flowNode?.id &&
//           node.targets.includes(flowNode?.id)
//         )
//       })
//       const forkBeginOfForwardNode = this.flowNodes.find((node) => {
//         return (
//           base?.component === 'LabelNode' &&
//           (node?.targets || []).length > 1 &&
//           base?.id &&
//           (node?.targets || []).includes(base?.id)
//         )
//       })
//       let baseNode = base
...
//         })
//       }
//       const cycleBackLabelNode = this.flowNodes.find(
//         (node) => node.loopBackTarget === flowNode?.id
//       )
...
//           return (
//             node?.targets?.length === 1 &&
//             cycleBackLabelNode?.id &&
//             node.targets.includes(cycleBackLabelNode?.id)
//           )
//         })
//       }
//       const cycleBeginOfEndNode = this.flowNodes.find(
//         (node) => node.loopEndTarget === flowNode?.id
//       )
...
//         const flowIdx = flow[meta.flowType]?.findIndex(
//           (flowMeta: FlowMetaParam) => flowMeta.id === meta.id
//         )
...
//               data.id,
//               meta.flowType,
//               currentFlow?.connector?.targetReference
//             )
//             // currentFlow.connector.targetReference = data.id
//             // this.initMetaFields(meta.flowType, currentFlow, MetaFieldType.EDIT, currentFlow.id)
//           }
...
//         } else if (
//           baseNode?.id === meta.id &&
//           !cycleBeginOfBackNode &&
//           baseNode?.component !== 'LabelNode'
//         ) {
//           if (meta.flowType === FlowMetaType.LOOP) {
//             if (metaType === FlowMetaType.LOOP && data?.defaultConnector) {
//               data.defaultConnector.targetReference =
//                 meta?.nextValueConnector?.targetReference || meta?.id || null
//             } else if (data.connector) {
//               const refValue =
//                 meta?.nextValueConnector?.targetReference || meta?.id || null
//               if (
//                 metaType === FlowMetaType.DECISION ||
//                 metaType === FlowMetaType.WAIT
//               ) {
//                 if (data.defaultConnector)
//                   data.defaultConnector.targetReference = refValue
...
//             if (metaType === FlowMetaType.LOOP && data?.defaultConnector) {
//               data.defaultConnector.targetReference =
//                 rules?.[idx]?.connector?.targetReference || null
//             } else if (data.connector) {
//               if (
//                 metaType === FlowMetaType.DECISION ||
//                 metaType === FlowMetaType.WAIT
...
//                 })
//               }
//               data.connector.targetReference =
//                 rules?.[idx]?.connector?.targetReference || null
//             }
//             this.initMetaFields(metaType, data, MetaFieldType.ADD)
//             const currentFlow = {
...
//             if (metaType === FlowMetaType.LOOP && data?.defaultConnector) {
//               data.defaultConnector.targetReference =
//                 flow[meta.flowType][flowIdx]?.defaultConnector
//                   ?.targetReference || null
//             } else if (data.connector) {
//               if (
//                 metaType === FlowMetaType.DECISION ||
...
//           if (
//             meta?.nextValueConnector?.targetReference === null &&
//             flowIdx > -1
//           ) {
...
//           } else {
//             this.updataCurrentFlowData(meta, data, metaType, true)
//             const labelNode = this.flowNodes.find(
//               (nd) => nd?.targets?.[0] === flowNode?.id
//             )
//             let node = labelNode
//             if (labelNode?.component === 'LabelNode') {
//               node = this.flowNodes.find(
//                 (node) => node?.loopEndTarget === flowNode?.id
//               )
//               if (!node) {
//                 node = this.flowNodes.find(
...
//               }
//             } else {
//               const forkNode = this.flowNodes.find(
//                 (node) => node?.decisionEndTarget === flowNode?.id
//               )
...
//             const mtIdx =
//               mTData?.flowType &&
//               flow[mTData?.flowType]?.findIndex(
//                 (flowMeta: FlowMetaParam) => flowMeta.id === mTData?.id
//               )
...
//               }
//               if (
//                 mTData.flowType === FlowMetaType.DECISION ||
//                 mTData.flowType === FlowMetaType.WAIT
//               ) {
//                 this.nextTargetFork(
//                   currentFlow,
//                   data.id,
...
//           if (metaType === FlowMetaType.LOOP && data?.defaultConnector) {
//             data.defaultConnector.targetReference =
//               meta?.defaultConnector?.targetReference || null
//           } else if (data.connector) {
//             if (
//               metaType === FlowMetaType.DECISION ||
//               metaType === FlowMetaType.WAIT
...
//               if (data.defaultConnector)
//                 data.defaultConnector.targetReference =
//                   meta?.connector?.targetReference || null
...
//   ) => {
//     if (
//       flowData?.defaultConnector?.targetReference === null ||
//       flowData?.defaultConnector?.targetReference === connectorId
//     ) {
...
//       if (
//         rule.connector?.targetReference === null ||
//         (rule.connector && targetReference === connectorId)
//       ) {
...
//     if (
//       node?.flowType === FlowMetaType.LOOP &&
//       node.defaultConnector?.targetReference === connectorId
//     ) {
...
//       if (
//         node.flowType === FlowMetaType.DECISION ||
//         node.flowType === FlowMetaType.WAIT
//       ) {
...
//   updataCurrentFlowData = (
//     meta: FlowMetaParamOfType,
//     data: FlowMetaParam,
//     metaType: FlowMetaType,
//     loopBack?: boolean
//   ) => {
...
//     } else {
//       if (metaType === FlowMetaType.LOOP && data?.defaultConnector) {
//         data.defaultConnector.targetReference = loopBack
//           ? meta.id
//           : meta?.connector?.targetReference || null
//       } else if (data.connector) {
//         if (
...
//           rules.forEach((rule: FlowMetaParam) => {
//             if (rule.connector)
//               rule.connector.targetReference = loopBack
//                 ? meta.id
//                 : meta?.connector?.targetReference || null
//           })
//         }
//         data.connector.targetReference = loopBack
//           ? meta.id
//           : meta?.connector?.targetReference || null
//       }
//     }
...
//   onInit = () => {
//     this.metaFlowDatas = []
//     const flowMeta = this.initialMeta.flow
//     for (const key in flowMeta) {
//       if (flowMeta.hasOwnProperty(key)) {
//         const metaType = showMetaTypes.find((type) => type === key)
//         this.initMetaFields(metaType, flowMeta)
//         if (metaType) {
//           if (isArr(flowMeta[key])) {
//             flowMeta[key].forEach((data: FlowMetaParam) => {
//               this.metaFlowDatas.push({
//                 ...data,
//                 flowType: metaType,
//               })
//             })
//           } else if (flowMeta[key]) {
//             this.metaFlowDatas.push({
//               ...flowMeta[key],
//               flowType: metaType,
//             })
...
//           this.flowAssignments = this.flowAssignments.filter(
//             (assignment) => assignment.id !== nodeId
//           )
//         } else {
...
//           })
//         } else if (metaFieldType === MetaFieldType.REMOVE) {
//           this.flowDecisions = this.flowDecisions.filter(
//             (decision) => decision.id !== nodeId
//           )
//         } else {
...
//           })
//         } else if (metaFieldType === MetaFieldType.REMOVE) {
//           this.flowSuspends = this.flowSuspends.filter(
//             (suspend) => suspend.id !== nodeId
//           )
//         } else {
...
//           })
//         } else if (metaFieldType === MetaFieldType.REMOVE) {
//           this.flowSortCollections = this.flowSortCollections.filter(
//             (sortCollention) => sortCollention.id !== nodeId
//           )
//         } else {
//           data[metaType]?.forEach(
//             (sortCollention: FlowSortCollectionProcessor) => {
//               this.flowSortCollections.push(
//                 new FlowSortCollectionProcessor(sortCollention)
//               )
//             }
//           )
//         }
//         this.mataFlowJson.flow[FlowMetaType.SORT_COLLECTION_PROCESSOR] =
//           this.flowSortCollections
//         break
//       // case FlowMetaType.RECORD_CREATE:
//       //   if (metaFieldType === MetaFieldType.ADD) {
...
//       //     });
//       //   } else if (metaFieldType === MetaFieldType.REMOVE) {
//       //     this.recordCreates = this.recordCreates.filter(
...
//       //     });
//       //   } else if (metaFieldType === MetaFieldType.REMOVE) {
//       //     this.recordDeletes = this.recordDeletes.filter(
//       //       (record) => record.id !== nodeId
//       //     );
//       //   } else {
...
//       //     });
//       //   } else if (metaFieldType === MetaFieldType.REMOVE) {
//       //     this.recordLookups = this.recordLookups.filter(
//       //       (record) => record.id !== nodeId
//       //     );
//       //   } else {
...
//       //     });
//       //   } else if (metaFieldType === MetaFieldType.REMOVE) {
//       //     this.recordUpdates = this.recordUpdates.filter(
//       //       (record) => record.id !== nodeId
//       //     );
//       //   } else {
...
//         this.mataFlowJson.flow[FlowResourceType.VARIABLE] = this.flowVariables
//         break
//       case FlowResourceType.CONSTANT:
//         this.flowConstants = data[metaType] || []
//         this.mataFlowJson.flow[FlowResourceType.CONSTANT] = this.flowConstants
//         break
//       case FlowResourceType.FORMULA:
//         this.flowFormulas = data[metaType] || []
//         this.mataFlowJson.flow[FlowResourceType.FORMULA] = this.flowFormulas
//         break
//       case FlowResourceType.TEMPLATE:
//         this.flowTemplates = data[metaType] || []
//         this.mataFlowJson.flow[FlowResourceType.TEMPLATE] = this.flowTemplates
//         break
//       default:
//         break
//     }
...
//   addFlowMeta = (metaType: FlowResourceType, flowMeta: IFieldMeta) => {
//     switch (metaType) {
//       case FlowResourceType.VARIABLE:
...
//         this.flowVariables = update(this.flowVariables, {
//           [index]: { $set: flowMeta },
//         })
//         this.mataFlowJson.flow[FlowResourceType.VARIABLE] = this.flowVariables
//         break
...
//         this.flowConstants = update(this.flowConstants, {
//           [index]: { $set: flowMeta },
//         })
//         this.mataFlowJson.flow[FlowResourceType.CONSTANT] = this.flowConstants
//         break
...
//         this.flowFormulas = update(this.flowFormulas, {
//           [index]: { $set: flowMeta },
//         })
//         this.mataFlowJson.flow[FlowResourceType.FORMULA] = this.flowFormulas
//         break
...
//         this.flowTemplates = update(this.flowTemplates, {
//           [index]: { $set: flowMeta },
//         })
//         this.mataFlowJson.flow[FlowResourceType.TEMPLATE] = this.flowTemplates
//         break
...
//   getFieldMetas = (fieldMetaType?: FlowResourceType) => {
...
//         }
//       } else if (
//         meta?.type === MetaValueType.OBJECT ||
//         meta?.type === MetaValueType.OBJECT_ID
//       ) {
//         const obj: IFieldMetaFlow = { ...meta }
...
//     //     webType: FlowResourceType.VARIABLE,
...
//     //   if (
//     //     !record.outputAssignments &&
//     //     (record.storeOutputAutomatically ||
//     //       (record.queriedFields && !record.outputReference))
//     //   ) {
//     //     const register = this.registers.find(
//     //       (reg) => reg.id === record.registerId
//     //     );
...
//     //         ? FlowResourceType.VARIABLE_RECORD
//     //         : FlowResourceType.VARIABLE_ARRAY_RECORD,
//     //       type: record.getFirstRecordOnly
...
//     //       };
//     //     }
//     //     const type = record.getFirstRecordOnly
//     //       ? FlowResourceType.VARIABLE_RECORD
//     //       : FlowResourceType.VARIABLE_ARRAY_RECORD;
...
//     fieldMetaType?: FlowResourceType
//   ) => {
//     const idx = fieldMetas.findIndex((meta) => meta?.value === type)
//     const meta = clone(fieldMeta)
//     // if (!meta.options && !fieldMetaType) {
//     //   meta.options = this.getFieldMetas(fieldMeta.type as FlowResourceType)
//     // }
//     const templateObj: any = {
//       [FlowResourceType.VARIABLE]: useLocale('flow.autoFlow.variable'),
//       [FlowResourceType.VARIABLE_RECORD]: useLocale(
//         'flow.autoFlow.variableRecord'
//       ),
//       [FlowResourceType.VARIABLE_ARRAY]: useLocale(
//         'flow.autoFlow.variableArray'
//       ),
//       [FlowResourceType.VARIABLE_ARRAY_RECORD]: useLocale(
//         'flow.autoFlow.variableArrayRecord'
//       ),
//       [FlowResourceType.CONSTANT]: useLocale('flow.autoFlow.constant'),
//       [FlowResourceType.FORMULA]: useLocale('flow.autoFlow.formula'),
//       [FlowResourceType.TEMPLATE]: useLocale('flow.autoFlow.template'),
//     }
//     // if ((fieldMetaType === FlowResourceType.VARIABLE_ARRAY
//     //   || fieldMetaType === FlowResourceType.VARIABLE_ARRAY_RECORD)
...
//   //   const metaNode = this.registers.find(
//   //     (reg) => reg.key === this.flowStart?.objectId
//   //   )
...
//   // getFormulaMap = (
//   //   formulaMeta: Record<string, IFieldMeta>,
//   //   meta: IFieldMeta,
//   //   isGlobal?: string
//   // ) => {
...
//   setFlowData = (
//     flowDatas: FlowMetaParamOfType[],
//     target?: FlowMetaParamOfType,
//     forkData?: FlowMetaParamOfType
//   ) => {
//     if (!target) {
...
//       })
//     } else if (
//       target?.flowType === FlowMetaType.DECISION ||
//       target?.flowType === FlowMetaType.WAIT
//     ) {
//       if (this.flowNodes.find((node) => node.id === target.id)) {
//         const rules = target?.rules || target?.waitEvents
//         rules?.some((rule) => {
//           const flowData = flowDatas.find(
...
//         if (defaultConnector) {
//           const flowData = flowDatas.find(
//             (data) => data.id === defaultConnector
//           )
//           if (flowData)
//             this.filterFlowData(defaultConnector, flowDatas, flowData, target)
//         }
//         if (targetReference) {
...
//     forkData?: FlowMetaParamOfType
//   ) => {
//     const flowData = this.metaFlowDatas.find(
//       (data) => data.id === target?.defaultConnector?.targetReference
//     )
...
//           return (
//             flowData?.connector?.targetReference === data.id &&
//             flowData?.connector?.targetReference !==
//               fork?.connector?.targetReference
//           )
//         } else if (flowData?.nextValueConnector) {
//           return (
//             flowData?.defaultConnector?.targetReference === data.id &&
//             flowData?.defaultConnector?.targetReference !==
//               fork?.connector?.targetReference
//           )
//         }
...
//       ? uid()
//       : flowData?.connector?.targetReference || this.flowEndId
//     this.flowNodes.push({
//       id: flowData.id,
//       type: 'begin',
//       width: STANDARD_SIZE,
...
//     flowData: FlowMetaParam,
//     metaType: FlowMetaType,
//     loopBackNode: FlowNode | undefined,
//     loopLastData?: FlowNode,
//     fork?: FlowMetaParam
//   ) {
//     const forkNode = this.flowNodes.find((node) => node.id === fork?.id)
//     const decisionEndId = forkNode?.decisionEndTarget
//     const endId = this.isEdit
//       ? uid()
//       : flowData?.connector?.targetReference || decisionEndId || this.flowEndId
...
//     if (!this.isEdit) {
//       const node = this.flowNodes.find(
//         (node) => node.id === flowData.connector?.targetReference
//       )
//       if (node) targetId = node.loopEndTarget as any
//     }
//     const decisionEndTarget = [
//       loopBackNode?.id
//         ? loopBackNode.id
//         : loopLastData?.loopBackTarget
//         ? loopLastData.loopBackTarget
//         : targetId,
//     ]
//     const decisions: FlowNode[] = [
//       {
//         id: flowData.id,
//         type: 'decisionBegin',
//         decisionEndTarget: loopBackNode?.id
//           ? loopBackNode.id
//           : loopLastData?.loopBackTarget
//           ? loopLastData.loopBackTarget
//           : endId,
...
//       const opartId = uid()
//       const connectorId =
//         rule?.connector?.targetReference !== fork?.connector?.targetReference &&
//         rule?.connector?.targetReference !==
//           flowData.connector?.targetReference &&
//         rule?.connector?.targetReference
//       decisions.push({
...
//         case FlowMetaType.DECISION:
//         case FlowMetaType.WAIT:
//           if (meta?.defaultConnector?.targetReference === flowData.id)
//             return meta.id
//           return this.getLoopBackId(
//             flowData,
//             meta?.defaultConnector?.targetReference
//           )
//         default:
...
//       const node = this.flowNodes.find(
//         (node) => node.id === flowData.defaultConnector?.targetReference
//       )
//       if (node) targetId = node.loopEndTarget as any
//     }
//     const loopEndTarget = [
...
//         : endId,
//     ]
//     const loops: FlowNode[] = [
//       {
//         id: flowData.id,
//         type: 'loopBegin',
//         loopBackTarget: this.isEdit ? backId : backLabelId,
//         loopEndTarget: this.isEdit
//           ? loopEndTarget.join('')
//           : loopLastData?.loopBackTarget || endLabelId,
//         width: STANDARD_SIZE,
//         height: STANDARD_SIZE,
//         targets: [
//           this.isEdit
//             ? backLabelId
//             : flowData.nextValueConnector?.targetReference || backLabelId,
//         ],
//         component: 'LoopNode',
//       },
//     ]
...
//       targets: [
//         this.isEdit
//           ? flowData?.nextValueConnector?.targetReference
//             ? id
//             : backId
//           : loopBackTarget !== loopLastData?.id
//           ? loopLastData?.loopBackTarget || endLabelId
//           : loopBackTarget || endLabelId,
//       ],
//       component: 'LabelNode',
//     })
...
//           this.isEdit
//             ? loopEndTarget.join('')
//             : loopLastData?.loopBackTarget || targetId,
//         ],
//         component: 'LabelNode',
//       })
//     }
//     if (flowData?.nextValueConnector?.targetReference && this.isEdit) {
//       const extendNode: FlowNode = {
//         id,
//         type: 'forward',
//         width: STANDARD_SIZE,
//         height: STANDARD_SIZE,
...
//     const forkNode = this.flowNodes.find((node) => node.id === fork?.id)
//     const decisionEndId = forkNode?.decisionEndTarget
//     let targetId = this.isEdit
//       ? loopBackNode
//         ? loopBackNode.id
//         : loopLastData?.loopBackTarget
//         ? loopLastData.loopBackTarget
//         : id
//       : targetNode?.connector?.targetReference ||
//         decisionEndId ||
//         this.flowEndId
//     if (!this.isEdit) {
//       const node = this.flowNodes.find(
//         (node) => node.id === targetNode.connector?.targetReference
//       )
//       if (node) targetId = node.loopBackTarget as any
//     }
//     const recordCreates: FlowNode[] = [
//       {
//         id: targetNode.id,
//         type: 'forward',
//         width: STANDARD_SIZE,
...
//   loopBackNode(targetNode: FlowMetaParam, flowNode?: FlowNode) {
...
//           if (target.id === flowNode?.id) targets?.splice(index, 1)
//         })
